...
export function EnhancedVehicleCard({ vehicle, className, pickupHubs = [] }: EnhancedVehicleCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-3 pb-0">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div className="flex flex-col gap-1 items-end shrink-0">
            {payAtPickup && (
              <Badge className="bg-primary text-primary-foreground text-[10px] px-1.5 py-0.5 gap-0.5 whitespace-nowrap">
                <Check className="h-2.5 w-2.5" />
                Pay at Pickup Available
              </Badge>
            )}
            {vehicle.rating > 0 && (
              <Badge variant="outline" className="bg-background text-foreground text-[10px] px-1.5 py-0.5 gap-0.5 border-yellow-400">
        ...
        <div className="flex items-center justify-end gap-1.5 mb-2">
          {homeDelivery && (
            <Badge className="bg-blue-500 text-white text-[10px] px-1.5 py-0.5 gap-0.5">
      ...
      {/* Image Gallery */}
      <div 
        className="relative aspect-[16/10] overflow-hidden bg-gradient-to-b from-muted/50 to-muted mx-3 rounded-md"
        ...
        {allImages.length > 1 && isHovering && (
          <>
            <button
              ...
              className="absolute left-1 top-1/2 -translate-y-1/2 bg-background/80 hover:bg-background text-foreground rounded-full p-1 shadow-md transition-all duration-200 hover:scale-110"
            >
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button
              onClick={handleNextImage}
              className="absolute right-1 top-1/2 -translate-y-1/2 bg-background/80 hover:bg-background text-foreground rounded-full p-1 shadow-md transition-all duration-200 hover:scale-110"
            >
      ...
      <div className="flex justify-center py-3">
        <Link to={`/vehicles/${vehicle.id}`}>
          <Button variant="outline" size="sm" className="text-xs font-medium border-border hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors duration-200">
            View All Packages
          </Button>
        </Link>
      </div>
      ...
      <div className="px-3 pb-3">
        {/* Specs */}
        <div className="flex items-center justify-center gap-3 text-xs text-muted-foreground border-t border-b border-border py-2 mb-3">
        ...
        <div className="flex items-end justify-between gap-2">
          <div>
            <p className="text-xl font-bold text-primary">
              ₹{vehicle.pricePerDay}
              <span className="text-[10px] font-normal text-muted-foreground ml-1">(incl. Tax)</span>
            </p>
          ...
          <Link to={`/booking/${vehicle.id}`}>
            <Button size="sm" className="bg-primary hover:bg-primary/90 text-xs px-4 transition-all duration-200 hover:shadow-md hover:scale-105 active:scale-100">
              Book Now
            </Button>
          </Link>
        </div>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between text-[10px] text-muted-foreground mt-3 pt-2 border-t border-border">
